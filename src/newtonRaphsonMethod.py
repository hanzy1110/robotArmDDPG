...
def newtonIter(q, J, dT, alpha=1):
    #Pseudo Inverse
    Jplus = np.linalg.inv(stabilize(J.T@J))@J.T
    return q-Jplus.dot(dT)*alpha
...
class NewtonControl:
    ...
    def get_angles(self, goal, actual):
        theta, tApprox = solve(goal,actual, 1000, 1e-3, ls=self.ls)
        return theta, tApprox

    def get_action(self,goal, actual, alpha=0.1):
        goal = jnp.array([goal['x'], goal['y']])
...
if __name__=="__main__":
    ...
    theta, tApprox = solve(objective, 1000, 1e-4, ls=ls)
